          ...
          {REASONS.map((reason) => {
            ...
            return (
              <motion.div
                key={reason.title}
                variants={fadeUpVariant}
                whileHover={{ y: -6, scale: 1.02, boxShadow: "0px 10px 20px rgba(0,0,0,0.08)" }}
                className="card-premium p-6 group bg-white border border-gray-100 rounded-2xl transition-colors cursor-default"
              >
                <div className={`w-12 h-12 ${reason.bg} rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-300`}>
